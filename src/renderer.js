...
let state = { items:[], settings:{historyDays:7,tournamentDays:7,autoSync:true}, sync:{}, notice:'' }, page = 'personal', query = '', statusFilter = 'all', eventFilter = 'all', selected = new Set(), lastNotice = '', toastTimer;
...
function isSourcePage() { return page === 'personal' || page === 'tournament'; }
function historyDays() { return Math.max(1,Math.min(365,Number(state.settings[page === 'tournament' ? 'tournamentDays' : 'historyDays']) || 7)); }
...
function currentItems() {
  return state.items.filter(x => {
    if (page === 'personal' || page === 'tournament') return x.category === page;
...
function row(r) {
  ...
  let host; try { host = new URL(r.url || r.sourceUrl).hostname; } catch { host = r.source === 'steam' ? 'Steam 官方' : r.source === 'hltv' ? 'HLTV' : '未知来源'; }
  ...
  if (['failed','interrupted','unavailable'].includes(r.status)) actions += button('record-source', '打开来源', 'subtle');
  ...
  if (r.status === 'completed') {
    ...
    if (r.files?.length) actions += button('play-command', '复制播放命令');
  ...
  const files = page === 'library' && r.files?.length > 1 ? `<div class="file-list">${r.files.map((f, index) => `<div class="file-entry"><span>${escape(f.split(/[\\/]/).pop())}</span><button class="button subtle small" data-action="play-command" data-id="${r.id}" data-index="${index}">复制播放命令</button></div>`).join('')}</div>` : '';
...
function emptyHTML() {
  ...
  if (isSourcePage()) {
    ...
    const title = sync.phase === 'syncing' ? '正在获取比赛记录' : login ? '登录一次，自动获取官匹记录' : verification ? '完成来源验证后继续获取' : `近 ${historyDays()} 天暂无已获取的比赛`;
    const description = sync.phase === 'syncing' ? '比赛记录会陆续显示，你可以留在这里查看进度。' : login ? '在 Steam 官方页面完成登录后，软件会保存本机登录凭证并自动获取比赛。' : verification ? '在来源窗口完成网站验证，然后刷新比赛。' : '可以扩大时间范围或刷新。已保存的历史记录和本地文件会继续保留。';
    return `<div class="empty"><div class="empty-icon">${page === 'tournament' ? '◇' : '↙'}</div><h3>${title}</h3><p>${description}</p>${sync.phase !== 'syncing' ? `<button class="button subtle" data-action="${login || verification ? 'verify-source' : 'sync'}">${login ? '登录 Steam 并自动获取' : verification ? '打开验证窗口' : '刷新比赛'} →</button>` : ''}</div>`;
  }
  const texts = {
    downloads:['下载队列空空如也','在个人比赛或赛事中心勾选 Demo 后，点击“下载所选”即可开始。','导入下载链接','import-dialog'],
    library:['你的复盘档案，从这里开始','下载完成的 Demo 会保存在这里。解压后可复制 CS2 播放命令。','查看个人比赛','personal'],
  };
...
function syncHTML() {
  ...
  const saved = page === 'personal' && state.auth?.steamSaved;
  const names = { idle:sync.lastSync ? '已同步' : '准备获取', syncing:'正在自动获取', login_required:'需要登录 Steam', verification_required:'需要网站验证', error:'获取未完成', partial:'已获取部分记录' };
  const primaryAction = ['login_required','verification_required'].includes(phase) ? 'verify-source' : 'sync';
  const primaryText = phase === 'login_required' ? '登录 Steam 并自动获取' : phase === 'verification_required' ? '打开验证窗口' : phase === 'syncing' ? '正在获取…' : '↻ 刷新比赛';
  const defaultMessage = page === 'personal' ? '默认获取近一周的官匹记录，包含优先、竞技与搭档模式。' : '自动获取近期已结束的比赛，并按赛事归类。选择比赛下载时会查找 Demo。';
  return `<div class="sync-state ${phase}"><div class="sync-copy"><div class="sync-title"><span class="sync-indicator ${phase}"></span><strong>${names[phase] || '获取比赛'}</strong>${saved ? '<span class="credential-badge">本机已加密保留登录</span>' : ''}</div><p id="syncMessage">${escape(sync.message || defaultMessage)}</p><div class="sync-meta">${sync.lastSync ? `上次同步 ${dateText(sync.lastSync, true)}` : '尚无成功同步记录'}${Number.isFinite(sync.found) && sync.lastSync ? `<b>·</b>最近获取 ${sync.found} 场` : ''}${saved && phase === 'login_required' ? '<b>·</b>登录已失效，请重新登录' : ''}</div></div><div class="sync-actions"><button class="button primary small" id="syncButton" data-action="${primaryAction}" ${phase === 'syncing' ? 'disabled' : ''}>${primaryText}</button>${['error','partial'].includes(phase) ? '<button class="button subtle small" data-action="verify-source">打开来源检查</button>' : ''}</div></div>`;
}
...
function renderPage() {
  const titleMap = { personal:['个人比赛','YOUR MATCHES. YOUR NEXT LEVEL.','每一场，都值得复盘','登录 Steam，自动获取近期官方比赛，选择你想复盘的每一场。'], tournament:['赛事中心','LEARN FROM THE BEST.','把顶级对局，收入囊中','自动获取已结束的比赛，按赛事、队伍与日期找到值得收藏的对局。'], downloads:['下载队列','YOUR REPLAYS, ON THE WAY.','精彩对局，正在抵达','管理下载进度、暂停和重试，所有文件保存在你的电脑上。'], library:['本地 Demo 库','A LIBRARY OF BETTER PLAYS.','你的私人复盘档案','整理已下载录像，解压文件，回到赛场的每一个关键瞬间。'], settings:['偏好设置','MAKE IT YOURS.','按你的习惯，开始','设置下载位置、自动获取与本机登录凭证。'], help:['使用指南','FROM DOWNLOAD TO REPLAY.','三步，回到比赛现场','自动获取比赛，保存 Demo，然后开始复盘。'] };
  ...
  $('#importButton').hidden = ['settings','help'].includes(page);
...
async function applyRange(days) {
  const category = page, key = category === 'tournament' ? 'tournamentDays' : 'historyDays';
  selected.clear();
...
function update(next) {
  ...
  const fetching = ['personal','tournament'].filter(category => state.sync?.[category]?.phase === 'syncing').length;
  $('#footerStatus').textContent = [count ? `${count} 个下载任务进行中` : '', fetching ? `正在获取${fetching === 2 ? '官匹与赛事' : state.sync?.personal?.phase === 'syncing' ? '官匹' : '赛事'}记录` : ''].filter(Boolean).join(' · ') || '就绪 · 所有记录保存在本机';
  ...
  if (page === 'settings') renderPage(); else renderRows();
}
...
document.addEventListener('click', async e => {
  ...
  try {
    ...
    else if (action === 'sync' || action === 'verify-source') await call(action, { category:page === 'tournament' ? 'tournament' : 'personal' });
